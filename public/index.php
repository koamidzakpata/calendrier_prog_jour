	...
	$start = $start->format('N') === '1' ? $start : (clone $month->getStartingDay())->modify('last monday');
		...
		<table class="calendar__table calendar__table--<?= $weeks; ?>weeks">
			<?php for ($i = 0; $i < $weeks; $i++): ?>
				<tr>
					...
					?>
						<td class="<?= $month->withInMonth($date) ? '' : 'calendar__overmonth'; ?> <?= $isToday ? 'is-today' : ''; ?>">
							<?php if ($i === 0): ?>
							...
							<?php foreach ($eventsForDay as $event): ?>
								<div class="calendar__event">
									...
									<a href="../public/edit.php?id=<?= $event['id']; ?>"><?= h($event['nom']); ?></a>
									|
									<a href="../public/delete.php?id=<?= $event['id']; ?>"
										class="text-danger"
